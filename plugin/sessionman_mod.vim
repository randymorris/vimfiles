"============================================================================"
"  XXX: This is not the official sessionman.vim!  This file contains personal
"  modifications that probably aren't suited for most people.
"
"  Vim session manager
"
...
function! s:ListSessions()
	" Always split below, act as q: window does
	let old_splitbelow = &splitbelow
	let &splitbelow = 1
	...
	syn match Comment "^\".*"
	put ='\" d - delete session, o - open session, e - edit session,'
	put ='\" x - edit extra session script, q - quit'
	put =''
	let l = line(".")
	...
	let &statusline = 'Session List' . &statusline

	" Only split as tall as necessary but not more than half
	if winheight(0) > line('$')
		exec "resize " . line('$')
		norm gg3j
	endif
	let &splitbelow = old_splitbelow
endfunction
